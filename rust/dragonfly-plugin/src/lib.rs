...
static HOST_API: AtomicPtr<dragonfly_plugin_sys::DfHostApiV20> =
...
pub unsafe fn install_host(host: *const dragonfly_plugin_sys::DfHostApiV20) {
...
fn read_skin_snapshot(
    host: &dragonfly_plugin_sys::DfHostApiV20,
    invocation: dragonfly_plugin_sys::DfInvocationId,
    snapshot: u64,
...
fn host_api() -> Option<&'static dragonfly_plugin_sys::DfHostApiV20> {
    unsafe { HOST_API.load(Ordering::Acquire).as_ref() }
}

fn read_item_stack(
    open: impl FnOnce(
        &dragonfly_plugin_sys::DfHostApiV20,
        *mut u64,
